# Codex Plugins Development

This repository contains custom Codex plugins. Each plugin is a self-contained directory with skills, agents, commands, and supporting resources.
...
├── AGENTS.md                 # Plugin-level context (optional)
└── README.md
```
...
description: >                  # Required. Max 1024 chars. What it does AND when to delegate to it.
  Describe when Codex should invoke this agent.
...
- CLI flags → Project (`.Codex/agents/`) → User (`~/.Codex/agents/`) → Plugin (`agents/`)
...
### AGENTS.md — Project/Plugin Context
...
The root `.Codex-plugin/marketplace.json` registers all plugins with metadata:
...
- **Skills vs Rules**: Skills are on-demand (loaded when relevant). Rules (`.Codex/rules/`) are always-on.
- **Skills vs AGENTS.md**: AGENTS.md is broad project context. Skills are focused, task-specific workflows.
