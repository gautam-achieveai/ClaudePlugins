...
Reference material for `code-reviewer:architecture-review`. The agent loads this
file **once it knows which structural areas the PR touches**, and reads **only
the relevant sections** rather than the whole catalog.

## Index

| § | Section | Typical severity |
|---|---------|------------------|
| 2.1 | Layer Boundary Violations | HIGH/CRITICAL |
| 2.2 | SOLID Principle Violations (SRP, DIP, LSP, ISP, OCP) | HIGH/MEDIUM |
| 2.3 | God Class / Excessive Responsibility | HIGH |
| 2.4 | Anemic Domain Model | LOW/MEDIUM |
| 2.5 | Component Coupling (circular, bounded context, fan-out, stamp, temporal, shared mutable state, hardcoded environment) | HIGH/MEDIUM |
