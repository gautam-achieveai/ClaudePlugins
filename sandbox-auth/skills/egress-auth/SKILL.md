...
`scripts/sandbox-auth-fetch.py` performs a request through the injected proxy + CA,
classifies the response, and polls through `auth_pending` with capped exponential backoff
until Allow, Deny, or timeout.
...
`scripts/warm-github-auth.py` drives the probes for you:
