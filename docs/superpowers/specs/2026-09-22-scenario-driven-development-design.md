...
2. **Regression and contract tests** — reproduce observed defects before fixing them, then protect stable material acceptance behavior at the cheapest layer.
...
### Stage 2 — Regression and contract tests

- Every defect found in Stage 1 gets a focused failing test before its fix; if authored later, show that it fails when the fix is reverted.
- After behavior stabilizes, every material acceptance contract gets at least one automated behavioral test at the **cheapest layer that still proves it** (unit over integration over end-to-end).
- One behavioral test may protect several scenarios; do not create a test catalogue that mirrors every interaction variant.
...
- Agents: `manual-tester` (scenario and contract ownership) and `developer` (defect regressions, behavioral contract tests, and coverage-gap tests).
...
- Guides: `writing-plans-guide.md` (task steps become: implement → run scenario → defect regression when needed → behavioral contract test → coverage check), `executing-plans-guide.md`.
- `debugging:systematic-debugging`: the bug-fix order above.
- Metadata: `development/plugin.json`, `marketplace.json`, `README.md`.
- External user-level instructions must mirror the current plugin roles and testing order when they override this workflow.

## Proof

- Contract tests fail first, then pass: the new skill exists with its three stages in order, material acceptance behavior is automated after stabilization, speed budgets and the coverage table remain, TDD opens with the opt-in banner, and no caller names TDD as its default.
