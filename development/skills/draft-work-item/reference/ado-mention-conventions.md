...
> **Loaded by** the `development:draft-work-item` skill when the active provider
> is Azure DevOps. (This is a self-contained copy of the `ado` plugin's
> conventions so the `development` plugin does not depend on `ado`.)
