...
Store: `docs/superpowers/learnings/<category>/<slug>.md`. Categories: `build`, `test`, `runtime`, `data`, `tooling`, `architecture`, `process`. One lesson per file. Slug: the title's key words in kebab-case, at most six (`sleep-hides-missing-await`).

`type`: `bug` when the lesson came from a failure and its fix; `knowledge` when it is a fact about the system found without a failure (a hidden constraint, a non-obvious dependency).
...
- **One lesson per finished task.** Several lessons means several runs, each through the gate.
...
At plan time (brainstorming, writing plans, agile step 0, subagent-driven development's plan read):

1. Grep frontmatter in `docs/superpowers/learnings/` for the change's component, tags, and the words in the request. No directory → skip, say so.
