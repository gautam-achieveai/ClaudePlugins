...
description: >
  ...
  — then delegates to gh:work-on or gh:babysit-pr accordingly. Tracks
...
Spawn a background agent that invokes `gh:work-on <workItemId>`.

The packet includes the issue's title, description, labels, milestone, project
fields, and linked items — but `gh:work-on` will fetch its own context anyway.
...
Spawn a background agent that invokes `gh:work-on <workItemId>` and pass the
...
re-fetch comments. `gh:work-on` auto-detects revision mode.
...
Spawn the `gh:babysit-pr-worker` agent for a **single pass** using the
