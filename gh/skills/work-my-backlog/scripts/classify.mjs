...
function isBotComment(comment) {
  if (typeof comment?.createdBy?.isBot === "boolean") {
    return comment.createdBy.isBot;
  }

  if (typeof comment?.createdBy?.type === "string" && comment.createdBy.type.length > 0) {
    return comment.createdBy.type === "Bot";
  }

  return /\[[^\]]*bot\]/i.test(comment?.text || "");
}

/**
 * Get human comments posted after a given timestamp.
 * Filters out bot comments like "[bot]" and "[Alice's bot]".
 */
export function extractHumanComments(comments, afterTimestamp) {
  ...
  return comments
    .filter((c) => {
      ...
      if (isBotComment(c)) return false;
      return true;
    })
