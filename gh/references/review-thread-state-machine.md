...
- **Deferral is valid**: Won't Fix with a deferred follow-up GitHub issue is
  acceptable (e.g., `Won't Fix: Deferred to #4567 for follow-up`). The reviewer
  can verify the issue exists.
...
3. Resolve or close the thread using the available GitHub review tooling
...
1. **Only resolve threads after fixing or replying** — in GitHub, either side may resolve a thread depending on permissions and repo settings
...
4. **Deferral is a valid Won't Fix** — if the developer creates a follow-up
   GitHub issue to track the concern and references its ID, that counts as a
   valid Won't Fix rationale. The reviewer should verify the issue exists before
   closing.
