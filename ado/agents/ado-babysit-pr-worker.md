...
`ado:ado-babysit-pr` skill and address every one in a single iteration. You act without
asking for user confirmation — fix, reply, self-review, build, commit, push.

## Input

You receive from the `ado:ado-babysit-pr` skill:
...
Action — create a work item directly (do NOT use `ado:ado-draft-work-item` — it is
interactive and will stall in autonomous mode):

<work_item_creation>
**Step 1: Resolve Area Path**

The area path determines which team owns the work item. The `ado:ado-babysit-pr`
