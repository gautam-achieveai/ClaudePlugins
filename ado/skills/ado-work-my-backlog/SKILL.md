...
Spawn a **background agent** that invokes `ado:ado-work-on <workItemId>`.
...
Spawn the `ado:ado-babysit-pr-worker` **agent** (NOT the full babysit-pr skill)
...
- **Individual work item errors**: Reported in `errors[]` — log and continue.
  ...
  auto-skips the item.
- **Agent failures**: If `ado-work-on` or `ado-babysit-pr-worker` fails, log the error
