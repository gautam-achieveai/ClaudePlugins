...
When re-entering `/work-on <id>` (Part 2), the bot must find and parse the
latest plan:
...
3. If NO human comments:
   → IMPLICIT APPROVAL (user re-ran /work-on, signaling readiness)
...
When the user re-runs `/work-on <id>` and there are zero human comments after
the latest bot plan comment, this is treated as implicit approval. The reasoning:
the user reviewed the plan on ADO, found it acceptable, and re-ran the command
to proceed.
...
of `/work-on <id>` regardless of further feedback.*
```
